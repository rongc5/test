...
namespace MZFRAME {
    ...
    int msg_passing_thread::find_channelid(pthread_t tid);
    {
    ...
    NET_OBJ * msg_passing_thread::gen_connect(const int fd, const sockaddr_in &addr)
    {
        ...
        p_connect = new base_connect<msg_passing_process>(fd, info);
        msg_passing_process *process = new msg_passing_process((NET_OBJ*)p_connect);
        
        base_connect<msg_passing_process> * tmp_con = (base_connect<msg_passing_process> *)p_connect;
    ...
    map<pthread_t, int> msg_passing_thread::_thd_channel_map;
