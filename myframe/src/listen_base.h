/*
 * FileName:       
 * Author:         wangqifeng  Version: 1.0  Date: 2012-6-1
 * Description:    
 * Version:        
 * Function List:  
 *                 1.
 ...
 */
#ifndef __LISTEN_BASE_HPP_
#define __LISTEN_BASE_HPP_
...
class listen_base:public base_net_obj
{
	public:
		listen_base()
		{
		...
		void init(const string &ip, unsigned short port)
		{
			...
			ret = listen(_sock, 250); /* Queue up to 50 connections before having them automatically rejected. */
			if (ret == -1)
			{
		...
		{	
			if ((events & EPOLLIN) == EPOLLIN)
			{
				...
				{				
					if (addr.sin_addr.s_addr == 0) //不知道为啥 有时候第一次通过accept得到的addr是空的，再调用getpeername来获取一次
					{
