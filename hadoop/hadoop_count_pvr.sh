...
$HOME_DIR/bin/hadoop jar $HOME_DIR/contrib/streaming/hadoop-streaming-1.0.3.jar \
 	-mapper  "python map_count_show.py"    \
	-reducer "python reduce_count_pvr.py"  \
	-file ./map_count_show.py \
	-file ./reduce_count_pvr.py \
  	-input /logs_lzo/xiaoshuo/$DATE/*/* \
  	-output $OUT_DIR2             \
  	-jobconf mapred.job.name='map_count_show.py'${DATE}    \
