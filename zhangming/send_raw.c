...
int main(void)
{
    ...
    sd = socket(AF_PACKET, SOCK_RAW, ETH_P_IP);
    if (-1 == sd)
    {
        perror("socket");
        exit(1);
    ...
    strcpy(packet.data, "qianfeng");
    build_ip_header(&packet);
    build_udp_header(&packet);

    memcpy(realpacket.e_hdr.ether_dhost, (void *)ether_aton("a4:ba:db:b0:f0:14"), ETH_ALEN);

    memcpy(realpacket.e_hdr.ether_shost, "123456789", ETH_ALEN);
    realpacket.e_hdr.ether_type = htons(ETHERTYPE_IP);

    ret = sendto(sd, &realpacket, sizeof(realpacket), 0, (struct sockaddr *)&dst, len);
