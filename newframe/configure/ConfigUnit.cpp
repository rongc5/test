...
namespace comcfg{
	...
	ErrCode ConfigUnit :: get_double(double * valueBuf)const{
		ul_writelog(Log::trace(), "Config : Get double from _key [%s]", _key.c_str());
		if(valueBuf == NULL){
			ul_writelog(Log::trace(), "Configure: visit ConfigUnit.get_double() failed : NULLBUFFER");
			setErrorKeyPath(this->_key);
			return NULLBUFFER;
		}
		int ret = Trans :: str2double(_value, valueBuf);
		if(ret) {
			ul_writelog(Log::trace(), "Configure: visit ConfigUnit.get_double() failed");
			setErrorKeyPath(this->_key);
		}
		return ret;
	}   
	...
	ErrCode ConfigUnit :: get_bsl_string(bsl_string * valueBuf) const{
		ul_writelog(Log::trace(), "Config : Get std::string from _key [%s]", _key.c_str());
		if(valueBuf == NULL){
			ul_writelog(Log::trace(), "Configure: visit ConfigUnit.get_bsl_string() failed : NULLBUFFER");
			setErrorKeyPath(this->_key);
			return NULLBUFFER;
		}
		int ret = Trans :: str2str(_value, valueBuf);
		if(ret) {
			ul_writelog(Log::trace(), "Configure: visit ConfigUnit.get_bsl_string() failed");
			setErrorKeyPath(this->_key);
		}
		return ret;
	}   
	...
	const bsl::string & ConfigUnit :: get_key_name() const {
		return _key;
	}
