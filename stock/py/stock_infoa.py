                ...
                if len(subitems) > 10:
                    if subitems[0] in deal_dic:
                        ...
                        if not len(deal_dic[subitems[0]]['last_single']['sum_vol_2']):
                            deal_dic[subitems[0]]['last_single']['sum_vol_2'].append(int(subitems[1]))
                        else:
                            deal_dic[subitems[0]]['last_single']['sum_vol_2'].append(int(subitems[1]) + deal_dic[subitems[0]]['last_single']['sum_vol_2'][-1])

                        if not len(deal_dic[subitems[0]]['last_single']['sum_vol_3']):
                            deal_dic[subitems[0]]['last_single']['sum_vol_3'].append(int(subitems[1]))
                        else:
                            deal_dic[subitems[0]]['last_single']['sum_vol_3'].append(int(subitems[1]) + deal_dic[subitems[0]]['last_single']['sum_vol_3'][-1])
...
def do_check_select(id_dic, sec, cf):
    ...
    for id_day in range(lastday_num):
        ...
        if cf.has_option(sec, 'lastday_has_down_pointer_ge') and id_dic.has_key('last_single') \
            and len(id_dic['last_single']['down_pointer']) > id_day:
            down_pointer = id_dic['last_single']['down_pointer'][id_day]
    ...
    if cf.has_option(sec, 'lastday_sum_vol_1_ge') and id_dic['last_single'].has_key('sum_vol_1') and \
                    len(id_dic['last_single']['sum_vol_1']) > lastday_num and lastday_num:
        if id_dic['last_single']['sum_vol_1'][lastday_num - 1] < cf.getfloat(sec, 'lastday_sum_vol_1_ge'):
            return

    if cf.has_option(sec, 'lastday_sum_vol_2_ge') and id_dic['last_single'].has_key('sum_vol_2') and \
                    len(id_dic['last_single']['sum_vol_2']) > lastday_num and lastday_num:
        if id_dic['last_single']['sum_vol_2'][lastday_num - 1] < cf.getfloat(sec, 'lastday_sum_vol_2_ge'):
            return

    if cf.has_option(sec, 'lastday_sum_vol_3_ge') and id_dic['last_single'].has_key('sum_vol_3') and \
                    len(id_dic['last_single']['sum_vol_3']) > lastday_num and lastday_num:
        if id_dic['last_single']['sum_vol_3'][lastday_num - 1] < cf.getfloat(sec, 'lastday_sum_vol_3_ge'):
            return

    if cf.has_option(sec, 'lastday_sum_range_percent_ge') and id_dic['last_single'].has_key('sum_range_percent') and \
                    len(id_dic['last_single']['sum_range_percent']) > lastday_num and lastday_num:
        if id_dic['last_single']['sum_range_percent'][lastday_num - 1] < cf.getfloat(sec, 'lastday_sum_range_percent_ge'):
            return
