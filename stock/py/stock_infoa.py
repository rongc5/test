                ...
                if len(subitems) > 10:
                    if subitems[0] in deal_dic:
                        ...
                        if not deal_dic[subitems[0]]['last_single'].has_key('vol_1'):
                            ...
                            deal_dic[subitems[0]]['last_single']['sum_range_percent'] = []
                            deal_dic[subitems[0]]['last_single']['sum_vol_1'] = []
                            deal_dic[subitems[0]]['last_single']['sum_vol_2'] = []
                            deal_dic[subitems[0]]['last_single']['sum_vol_3'] = []
                            deal_dic[subitems[0]]['last_single']['down_pointer'] = []
                        ...
                        if abs(float(subitems[8]) - float(subitems[7])) > 0:
                            down_pointer = round(abs(float(subitems[8]) - float(subitems[9])) * 1.0 /abs(float(subitems[8]) - float(subitems[7])), 2)
                        else:
                            down_pointer = 0
                        deal_dic[subitems[0]]['last_single']['down_pointer'].append(down_pointer)
...
def log_print_res(id_dic, file_name):
    if not len(id_dic):
        return
    ...
    day = Day()
    today = '%s' % (day.today())
    file_name = '%s%s' % (file_name, today.replace('-', ''))

    log_write(file_name, 'begin ==========:%s' % (day.datetime()))
    ...
    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('total_vol'):
        search_dic['last_single'].pop('total_vol')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('start'):
        search_dic['last_single'].pop('start')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('end'):
        search_dic['last_single'].pop('end')

    #if search_dic.has_key('last_single') and search_dic['last_single'].has_key('low'):
    #    search_dic['last_single'].pop('low')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('high'):
        search_dic['last_single'].pop('high')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('sum_vol_3'):
        search_dic['last_single'].pop('sum_vol_3')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('sum_vol_2'):
        search_dic['last_single'].pop('sum_vol_2')

    if search_dic.has_key('last_single') and search_dic['last_single'].has_key('sum_vol_1'):
        search_dic['last_single'].pop('sum_vol_1')

    #if search_dic.has_key('single') and search_dic['single'].has_key('ratio_vol_3'):
    #    search_dic['single'].pop('ratio_vol_3')
    #
    #if search_dic.has_key('single') and search_dic['single'].has_key('ratio_vol_2'):
    #    search_dic['single'].pop('ratio_vol_2')
    #
    #if search_dic.has_key('single') and search_dic['single'].has_key('ratio_vol_1'):
    #    search_dic['single'].pop('ratio_vol_1')

    #print file_name, search_dic
    log_write(file_name, json.dumps(search_dic))
    log_write(file_name, '\n')
    log_write(file_name, 'serch over ==========')
...
def do_check_remove(id_dic, sec, cf):
    ...
    if cf.has_option(sec, 'circulation_market_value_ge') and id_dic.has_key('circulation_market_value') and id_dic['circulation_market_value'] < cf.getfloat(sec, 'circulation_market_value_ge'):
        return True

    if cf.has_option(sec, 'circulation_market_value_le') and id_dic.has_key('circulation_market_value') and id_dic['circulation_market_value'] > cf.getfloat(sec, 'circulation_market_value_le'):
        return True

    if cf.has_option(sec, 'end_le') and id_dic.has_key('end') and float(id_dic['end']) > cf.getfloat(sec, 'end_le'):
        return True

    if cf.has_option(sec, 'end_ge') and id_dic.has_key('end') and float(id_dic['end']) < cf.getfloat(sec, 'end_ge'):
        return True

    if cf.has_option(sec, 'pe_le') and id_dic.has_key('pe') and float(id_dic['pe']) > cf.getfloat(sec, 'pe_le'):
        return True

    if cf.has_option(sec, 'pe_ge') and id_dic.has_key('pe') and float(id_dic['pe']) < cf.getfloat(sec, 'pe_ge'):
        return True
...
def do_check_select(id_dic, sec, cf):
    ...
    lastday_num = 0
    if id_dic['last_single'].has_key('sum_vol_1'):
        lastday_num = len(id_dic['last_single']['sum_vol_1'])

        id_dic['last_single']['sum_vol_1_with_now'] = []
        id_dic['last_single']['sum_vol_2_with_now'] = []
        id_dic['last_single']['sum_vol_3_with_now'] = []
        id_dic['last_single']['lastday_end_diff_low'] = []

    for id_day in range(lastday_num):
        diff = 0

        if id_dic.has_key('end') and id_dic['last_single'].has_key('low'):
            diff = round((id_dic['end'] - id_dic['last_single']['low'][id_day]) /id_dic['end'], 2)

        id_dic['last_single']['sum_vol_1_with_now'].append(id_dic['last_single']['sum_vol_1'][id_day] + id_dic['single']['vol_1'][-1] )
        id_dic['last_single']['sum_vol_2_with_now'].append(id_dic['last_single']['sum_vol_2'][id_day] + id_dic['single']['vol_2'][-1] )
        id_dic['last_single']['sum_vol_3_with_now'].append(id_dic['last_single']['sum_vol_3'][id_day] + id_dic['single']['vol_3'][-1] )
        id_dic['last_single']['lastday_end_diff_low'].append(diff)
    ...
    if cf.has_option(sec, 'circulation_market_value_ge') and id_dic.has_key('circulation_market_value') and float(id_dic['circulation_market_value']) < cf.getfloat(sec, 'circulation_market_value_ge'):
        return ret
    ...
    if  cf.has_option(sec, 'diff_vol_1_ge') and (len(id_dic['single']['diff_vol_1']) ==0 or id_dic['single']['diff_vol_1'][-1] < cf.getfloat(sec, 'diff_vol_1_ge')):
        return ret

    if  cf.has_option(sec, 'diff_vol_2_ge') and (len(id_dic['single']['diff_vol_2']) == 0 or id_dic['single']['diff_vol_2'][-1] < cf.getfloat(sec, 'diff_vol_2_ge')):
        return ret

    if  cf.has_option(sec, 'diff_vol_3_ge') and (len(id_dic['single']['diff_vol_3']) == 0 or id_dic['single']['diff_vol_3'][-1] < cf.getfloat(sec, 'diff_vol_3_ge')):
        return ret
    ...
    for id_day in range(lastday_num):
        ...
        if cf.has_option(sec, 'lastday_sum_vol_1_ge') and id_dic['last_single']['sum_vol_1_with_now'][id_day] > cf.getfloat(sec, 'lastday_sum_vol_1_ge'):
            lastday_sum_vol += 1

        if cf.has_option(sec, 'lastday_sum_vol_2_ge') and id_dic['last_single']['sum_vol_2_with_now'][id_day] > cf.getfloat(sec, 'lastday_sum_vol_2_ge'):
            lastday_sum_vol += 1

        if cf.has_option(sec, 'lastday_sum_vol_3_ge') and id_dic['last_single']['sum_vol_3_with_now'][id_day] > cf.getfloat(sec, 'lastday_sum_vol_3_ge'):
            lastday_sum_vol += 1

        if lastday_sum_vol > 0:
            break

    if flag and lastday_sum_vol <= 0:
        return ret

    ret = 1
    print id_dic
    id_dic['sec'] = sec
    log_print_res(id_dic, 'res_list')
...
def do_search_short():
    ...
    while 1:
        ...
        if monitor_mtime != mtime:
            ...
            get_last_singles(10, id_dic)

        day = Day()
        if int(day.hour) >= 15:
            #print day.hour
            log_single_analysis(load_base_list())
        ...
        for key in id_dic:
            ...
            for sec in cf.sections():
                if 'remove' in sec:
                    ...
                    if do_check_select(id_dic[key], sec, cf):
                        break

            res_str = '%s\t%d\t%d\t%d\t%d\t%.2f\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f' % (key, id_dic[key]['single']['vol_1'][-1], id_dic[key]['single']['vol_2'][-1], id_dic[key]['single']['vol_3'][-1], \
            0, 0, id_dic[key]['circulation_market_value'], id_dic[key]['start'], id_dic[key]['end'], id_dic[key]['low'], \
            id_dic[key]['high'], id_dic[key]['range_percent'], id_dic[key]['up_pointer'], id_dic[key]['down_pointer'])
            log_write('singles_tmp', res_str)
