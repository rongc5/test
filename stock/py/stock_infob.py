...
def do_search_short():
    ...
    while 1:
        ...
        for key in id_dic:
            ...
            if  not id_dic[key].has_key('range_percent') or not id_dic[key].has_key('big_res'):
                continue
            index = random.randint(0, len(user_agent_list) -1)
...
class TodoHandler(BaseHTTPRequestHandler):
    ...
    def inert_data(self, post_values):
        ...
        if not self.id_dic.has_key(post_values['id']):
            ...
            if post_values.has_key('change_rate'):
                ...
                self.id_dic[key]['down_pointer'] = post_values['down_pointer']
                self.id_dic[key]['up_pointer'] = post_values['up_pointer']
        else:
...
#http://127.0.0.1:8082/search?change_rate_ge=1&range_percent_ge=-3.9&range_percent_le=4.9&big_res=1&big_res2=1&big_res_ge=1000&big_res2_ge=1000&res_vol_ratio_ge=0.05&
#        res2_vol_ratio_ge=0.05&down_pointer_ge=0.8

        search_res = {}
        for key in self.id_dic:
            if query_components.has_key('change_rate_ge'):
                if self.id_dic[key]['change_rate'] < query_components['change_rate_ge']:
                    continue

            if query_components.has_key('range_percent_ge'):
                if self.id_dic[key]['range_percent'] < query_components['range_percent_ge']:
                    continue

            if query_components.has_key('range_percent_le'):
                if self.id_dic[key]['range_percent'] > query_components['range_percent_le']:
                    continue

            if query_components.has_key('big_res'):
                if not get_data_direction(self.id_dic[key]['big_res']):
                    continue

            if query_components.has_key('big_res2'):
                if not get_data_direction(self.id_dic[key]['big_res2']):
                    continue

            if query_components.has_key('big_res_ge'):
                if self.id_dic[key]['big_res'][-1] < query_components['big_res']:
                    continue

            if query_components.has_key('big_res2_ge'):
                if self.id_dic[key]['big_res2'][-1] < query_components['big_res2']:
                    continue

            if query_components.has_key('res_vol_ratio_ge'):
                if self.id_dic[key]['res_vol_ratio'][-1] < query_components['res_vol_ratio']:
                    continue

            if query_components.has_key('res2_vol_ratio_ge'):
                if self.id_dic[key]['res2_vol_ratio'][-1] < query_components['res2_vol_ratio']:
                    continue

            if query_components.has_key('down_pointer_ge'):
                if self.id_dic[key]['down_pointer'][-1] < query_components['down_pointer']:
                    continue

            if query_components.has_key('up_pointer_le'):
                if self.id_dic[key]['up_pointer'][-1] > query_components['up_pointer']:
                    continue

            search_res[key] = self.id_dic[key]

        # Just dump data to json, and return it
        message = json.dumps(search_res)
