...
bool hq_sum_change_rate_search_index::search(std::string &key, std::string &value, std::set<std::string> & search)
{
   if (!key.compare("hq_sum_change_rate_ge") || !key.compare("hq_sum_change_rate_ge_v"))
   {
       return do_check_hq_sum_change_rate_ge(key, value, search);
   }
   else if (!key.compare("hq_sum_change_rate_le") || !key.compare("hq_sum_change_rate_le_v"))
   {
       return do_check_hq_sum_change_rate_le(key, value, search);
   }

   return false;
}

bool hq_sum_change_rate_search_index::do_check_hq_sum_change_rate_le(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
