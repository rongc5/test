...
bool hqup_pointer_search_index::search(std::string &key, std::string &value, std::set<std::string> & search)
{
   if (!key.compare("hqup_pointer_ge") || !key.compare("hqup_pointer_ge_v"))
   {
       return do_check_hqup_pointer_ge(key, value, search);
   }
   else if (!key.compare("hqup_pointer_le") || !key.compare("hqup_pointer_le_v"))
   {
       return do_check_hqup_pointer_le(key, value, search);
   }
   else if (!key.compare("hqup_pointer_ge_num_ge") || !key.compare("hqup_pointer_ge_num_ge_v"))
   {
       return do_check_hqup_pointer_ge_num_ge(key, value, search);
   }

   return false;
}

bool hqup_pointer_search_index::do_check_hqup_pointer_le(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
...
bool hqup_pointer_search_index::do_check_hqup_pointer_ge(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
    std::multimap<float, std::string>::iterator it_le, it_ge, it;
    std::map<std::string, std::multimap<float, std::string> >  * search_index = current();
...
bool hqup_pointer_search_index::do_check_hqup_pointer_ge_num_ge(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
    std::multimap<float, std::string>::iterator it_le, it_ge, it;
    std::map<std::string, std::multimap<float, std::string> >  * search_index = current();
