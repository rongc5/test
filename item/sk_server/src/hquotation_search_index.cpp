        ...
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                for (auto date_index_end: date_index_end_set)
                {
                    ...
                    p_data->get_search_sstr(ii->first, "lowest", date_index, date_index_end, search, index);
                    if (index.empty()) 
                        continue;
        ...
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                for (auto date_index_end: date_index_end_set)
                {
                    ...
                    p_data->get_search_sstr(ii->first, "trough", date_index, date_index_end, search, index);
                    if (index.empty()) 
                        continue;
        ...
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                for (auto date_index_end: date_index_end_set)
                {
                    ...
                    p_data->get_search_sstr(ii->first, "highest", date_index, date_index_end, search, index);
                    if (index.empty()) 
                        continue;
        ...
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                for (auto date_index_end: date_index_end_set)
                {
                    ...
                    p_data->get_search_sstr(ii->first, "crest", date_index, date_index_end, search, index);
                    if (index.empty()) 
                        continue;
...
bool hquotation_search_index::do_check_hqhigh_end_5_ge(std::string &name, std::string &value, search_res & search)
{
    hquotation_search_item * search_index = current();
    proc_data* p_data = proc_data::instance();
...

    {
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                ...
                if (tt[len - date_index - 1]->high_end_5 >= hi.fpoint)
                    search.append(name, ii->first, date_index);
            }
        }
    }
...
bool hquotation_search_index::do_check_hqlow_end_5_ge(std::string &name, std::string &value, search_res & search)
{
    hquotation_search_item * search_index = current();
    proc_data* p_data = proc_data::instance();
...

    {
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                ...
                if (tt[len - date_index - 1]->low_end_5 >= hi.fpoint)
                    search.append(name, ii->first, date_index);
            }
        }
    }
...
bool hquotation_search_index::do_check_hqhigh_end_5_le(std::string &name, std::string &value, search_res & search)
{
    hquotation_search_item * search_index = current();
    proc_data* p_data = proc_data::instance();
...
bool hquotation_search_index::do_check_hqlow_end_5_le(std::string &name, std::string &value, search_res & search)
{
    hquotation_search_item * search_index = current();
    proc_data* p_data = proc_data::instance();
