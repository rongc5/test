...
int plate_dict::init(const char * path, const char * file, const char *dump_dir)
{
...
void plate_dict::set_path (const char* path)
{
    snprintf(_fullpath, sizeof(_fullpath), "%s", path);
}

bool plate_dict::need_reload()
{
    struct stat st;
...
int plate_dict::dump()
{
    ...
    ASSERT_WARNING(fp != NULL, "plate_dict dump_data failed, open file [%s] error", _dumppath);
