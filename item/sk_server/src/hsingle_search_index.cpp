...
bool hsingle_search_index::do_hsratio_le(std::string &name, std::string &value, search_res & search)
{
    ...
    hi.date_index = t_vec[0];
    hi.single_index = atoi(t_vec[1].c_str());
    hi.single = atof(t_vec[2].c_str()) * 10000;
...

    {
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
            ...
            for (auto date_index: date_index_set)
            {
                ...
                if ((int) tt[len - abs(date_index) - 1].back()->at(hi.single_index).dratio * 10000 <= hi.single)
                    search.append(name, ii->first, date_index);
            }
        }
    }
...
bool hsingle_search_index::do_hsratio_ge(std::string &name, std::string &value, search_res & search)
{
    ...
    hi.date_index = t_vec[0];
    hi.single_index = atoi(t_vec[1].c_str());
    hi.single = atoi(t_vec[2].c_str()) * 10000;
...
bool hsingle_search_index::do_hsratio_ge_num_ge(std::string &name, std::string &value, search_res & search)
{
    ...
    hi.sum = atoi(t_vec[2].c_str());
    hi.single_index = atoi(t_vec[3].c_str());
    hi.single = atoi(t_vec[4].c_str()) * 10000;
        ...
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
                ...
                for (auto date_index_end: date_index_end_set)
            {
                ...
                for (int k = len - abs(date_index) - 1; k <= len - abs(date_index_end) - 1; k++) 
                {
                    ...
                    if ((int) tt[k].back()->at(hi.single_index).dratio * 10000 >= hi.single)
                    {
                        count++;
                        index.insert(len - k - 1);
                    }
...
bool hsingle_search_index::do_hsratio_sum_ge(std::string &name, std::string &value, search_res & search)
{
    ...
    hi.date_index_end = t_vec[1];
    hi.single_index = atoi(t_vec[2].c_str());
    hi.single = atoi(t_vec[3].c_str()) * 10000;
...

    {
        for (auto k = search._id_sets.begin(); k != search._id_sets.end(); k++)
        {
                ...
                for (auto date_index_end: date_index_end_set)
            {
                ...
                unsigned long long sum = ii->second[len - abs(date_index_end) -1]->at(hi.single_index).in + ii->second[len - abs(date_index_end) -1]->at(hi.single_index).out;
                unsigned long long dsum = ii->second[k]->at(hi.single_index).in + ii->second[k]->at(hi.single_index).out;
                sum = dsum && sum? sum - dsum: 0;
                int diff = ii->second[len - abs(date_index_end) -1]->at(hi.single_index).diff - ii->second[k]->at(hi.single_index).diff;
                float dratio = sum ? diff * 1.0 / sum : 0;

                if ((int) dratio * 10000 >= hi.single)
                {
                    search.append(name, ii->first, index);
                }
            }
...
bool hsingle_search_index::do_hsratio_sum_le(std::string &name, std::string &value, search_res & search)
{
