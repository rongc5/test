...
real_quotation_dict::real_quotation_dict()
{
}
...
int real_quotation_dict::init(const char * path, const char * file, const char *dump_dir)
{
...
void real_quotation_dict::set_path (const char* path)
{
    snprintf(_fullpath, sizeof(_fullpath), "%s", path);
}

bool real_quotation_dict::need_reload()
{
    struct stat st;
...
int real_quotation_dict::dump()
{
    //FILE * fp = fopen(_dumppath, "w");
    //ASSERT_WARNING(fp != NULL, "real_quotation_dict dump_data failed, open file [%s] error", _dumppath);
