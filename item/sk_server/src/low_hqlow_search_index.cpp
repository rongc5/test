#include "low_hqlow_search_index.h"
#include "sk_util.h"
#include "history_quotation_dict.h"
#include "proc_data.h"

bool low_hqlow_search_index::search(std::string &key, std::string &value, std::set<std::string> & search)
{
   if (!key.compare("low_hqlow_ge") || !key.compare("low_hqlow_ge_v"))
   {
       return do_check_low_hqlow_ge(key, value, search);
   }
   else if (!key.compare("low_hqlow_le") || !key.compare("low_hqlow_le_v"))
   {
       return do_check_low_hqlow_le(key, value, search);
   }

   return false;
}

bool low_hqlow_search_index::do_check_low_hqlow_le(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
...
bool low_hqlow_search_index::do_check_low_hqlow_ge(std::string &key, std::string &value, std::set<std::string> & search)
{
    float end = 0;
    std::multimap<float, std::string>::iterator it_le, it_ge, it;
    std::map<std::string, std::multimap<float, std::string> >  * search_index = current();
