...
{
   if (start_with(key, "hsingle_sum_diff_0_ge") || start_with(key, "hsingle_sum_diff_1_ge") || start_with(key, "hsingle_sum_diff_2_ge"))
   {
       return do_check_hsingle_sum_diff_ge(key, value, search);
   }
   else if (start_with(key, "hsingle_sum_diff_0_le") || start_with(key, "hsingle_sum_diff_1_le") || start_with(key, "hsingle_sum_diff_2_le"))
   {
       return do_check_hsingle_sum_diff_le(key, value, search);
   }

   return false;
}

bool hsingle_sum_diff_search_index::do_check_hsingle_sum_diff_le(std::string &key, std::string &value, std::set<std::string> & search)
{
    uint32_t index = 0;
    int end = 0;
...
bool hsingle_sum_diff_search_index::do_check_hsingle_sum_diff_ge(std::string &key, std::string &value, std::set<std::string> & search)
{
    uint32_t index = 0;
    int end = 0;
    std::multimap<int, std::string>::iterator it_le, it_ge, it;
