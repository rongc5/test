...
int Proc_data::init(sk_conf * conf)
{
    ...
    hs_dict1->init(_conf->id_path.c_str());
    ...
    hs_dict2->init(_conf->id_path.c_str());
    ...
    hq_dict2->init(_conf->id_path.c_str());

    _hquoation_dict = new (std::nothrow)reload_mgr<history_quotation_t>(id_dict1, id_dict2);
}
