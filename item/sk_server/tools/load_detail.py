...
DATAPATH = BASE_DATA_PATH + 'details'
SINGLEPATH = BASE_DATA_PATH + 'singles'
SINGLE_DICT_PATH_TMP = BASE_DATA_PATH + 'single_dict_tmp'
SINGLE_DICT_PATH = BASE_DATA_PATH + 'single_dict'
...
if __name__ == '__main__':
    ...
    load_coad_all()
    load_holiday_dict()
    load_details(2, id_all)

    day_list = load_days(20)
    day_arr = []
    for day in day_list:
    ...
    if len(day_arr):
        path = '%s/%s' % (DATAPATH, day_arr[0])
        cmd = ['python', CURRENT_DIR_PATH + '/count_detail.py', path]
        subprocess.Popen(cmd, stdout=subprocess.PIPE).communicate()[0]

    for day  in day_arr:
        single_path = '%s/singles_%s' % (SINGLEPATH, day)
        log_write(SINGLE_DICT_PATH_TMP, os.path.abspath(single_path))

    if os.path.isfile(SINGLE_DICT_PATH_TMP):
        cmd = ['mv', SINGLE_DICT_PATH_TMP, SINGLE_DICT_PATH]
        subprocess.Popen(cmd, stdout=subprocess.PIPE).communicate()[0]
