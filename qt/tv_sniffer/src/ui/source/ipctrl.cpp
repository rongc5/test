...
void IPCtrl::MovePrevLineEdit(int i)
{
 ...
 QString IPCtrl::get_ip ()
 {
     if (m_pLineEdit[0]->text ().isEmpty ()
         || m_pLineEdit[1]->text ().isEmpty ()
         || m_pLineEdit[2]->text ().isEmpty ()
         || m_pLineEdit[3]->text ().isEmpty ())
     {
         return QString ();
     }

     return QString (m_pLineEdit[0]->text ())
         .append(".")
         .append(m_pLineEdit[1]->text ())
         .append(".")
         .append(m_pLineEdit[2]->text ())
         .append(".")
         .append(m_pLineEdit[3]->text ());

 }
 void IPCtrl::set_ip (const QString & ip)
 {
     QStringList ip_list = ip.split (".");
     if (ip_list.count() == 4)
     {
        m_pLineEdit[0]->setText (ip_list[0]);
        m_pLineEdit[1]->setText (ip_list[1]);
        m_pLineEdit[2]->setText (ip_list[2]);
        m_pLineEdit[3]->setText (ip_list[3]);
