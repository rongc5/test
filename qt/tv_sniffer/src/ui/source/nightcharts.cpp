...
int Nightcharts::draw(QPainter *painter)
{
    ...
    else if (this->ctype==Nightcharts::Histogramm)
    {
        ...
        for (int i=1;i<10;i++)
        {
            painter->drawLine(cX-3,cY+cH/10*i,cX+3,cY+cH/10*i);    //溴脲龛 镱 铖?Y
            //painter->drawText(cX-20,cY+cH/10*i,QString::number((10-i)*10)+"%");
        }
        painter->drawLine(cX,cY+cH,cX,cY);         //铖?Y
        painter->drawLine(cX,cY,cX+4,cY+10);       //耱疱腙?
        painter->drawLine(cX,cY,cX-4,cY+10);
        painter->drawLine(cX,cY+cH,cX+cW,cY+cH);   //铖??
